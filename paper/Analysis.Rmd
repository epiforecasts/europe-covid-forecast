...
score_forecasts <- function(data, 
  ...
  eval_forecasts(data, 
                 ...
                 by = c("forecast_date", "target_type", 
                        "horizon", "model", "expert"))[]
}
...
scores_all_range <- score_forecasts(log_data, 
                                    summarise_by = c("model", "forecast_date", 
                                                     "target_type", "expert", 
                                                     "range"))
...
plot_coverage <- function(df, interval = 50, target = "Cases") {
  df <- scores_all_range %>%
    group_by(target_type, model, range) %>%
    summarise(coverage = mean(coverage)) %>%
    filter(range %in% interval, 
           target_type == target) 
  
  df %>%
  ggplot(aes(y = coverage, x = reorder(model, -coverage), 
             group = range)) + 
...
n_forecasts$model %>% unique() %>% length()
```
