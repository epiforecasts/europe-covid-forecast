...
### Crowdforecastr webiste
Our forecasts now have their own website, [www.crowdforecastr.org](www.crowdforecastr.org). You can find all relevant information there, including a short list of suggestions on how to make better forecasts. 

You can now also access the forecasts via [app.crowdforecastr.org](app.crowdforecastr.org). 

### Reminder: make a forecast until Monday 11pm CET

We **update our data every Sunday at 8.30am CET**. You can then make a forecast until **11pm CET (1opm UK time) on Monday** using our [forecast app](https://cmmid-lshtm.shinyapps.io/crowd-forecast/). We would be very happy to have a forecast from you this week!
...
Now, let's have a look at last weeks forecasts and performance! Here is an overview of last week's forecasts for the locations you forecasted and the ensemble forecast. Depending on the number of locations you forecasted, this might look a bit crammed. You can find more detailed information on our performance board [here](https://epiforecasts.io/europe-covid-forecast). 


```{r plot-forecasts, echo=FALSE, out.width="49%", fig.show="hold"}

if (weekdays(Sys.Date()) == "Monday") {
  last_forecast_date <- covid.german.forecasts::latest_weekday(Sys.Date() - 7)
} else {
  last_forecast_date <- covid.german.forecasts::latest_weekday(Sys.Date())
}
