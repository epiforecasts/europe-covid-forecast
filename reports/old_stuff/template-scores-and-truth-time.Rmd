...
for (loc in locations) {
  ...
  # plot overprediction
  p3 <- tmp_scores  %>%
    ggplot(aes(x = forecast_date, y = overprediction, colour = model)) + 
    geom_line() + 
    geom_point() + 
    theme_light() + 
    facet_wrap(~ target_variable, scales = "free") + 
  ...
  cat("\n\n#### Underprediction")
  p4 <- tmp_scores  %>%
    ggplot(aes(x = forecast_date, y = underprediction, colour = model)) + 
    geom_line() + 
    geom_point() + 
    theme_light() + 
    facet_wrap(~ target_variable, scales = "free") + 
  ...
  cat("\n\n#### Sharpness")
  p5 <- tmp_scores  %>%
    ggplot(aes(x = forecast_date, y = sharpness, colour = model)) + 
    geom_line() + 
    geom_point() + 
    theme_light() + 
    facet_wrap(~ target_variable, scales = "free") + 
