...
Below you find a visualisation of all forecasts made for the [UK Covid-19 Crowd Forecasting Challenge](https://www.crowdforecastr.org/2021/05/11/uk-challenge/) as well as an evaluation of the forecasts. The latter part will be added as soon as data comes in. 
...
table <- eval_forecasts(rankingdata, summarise_by = c("model"), 
                        compute_relative_skill = TRUE)


table <- table[, c("model", "relative_skill")]
...
out <- htmltools::tagList(
  table %>%
    ...
    dplyr::rename(`relative skill (lower = better)` = relative_skill, 
                  forecaster = model) %>%
