``` {r estimate-heigth}
fd <- max(as.Date(forecast_dates))

n_models <- data %>%
  dplyr::filter(location_name %in% loc) %>%
  dplyr::group_by(forecast_date) %>%
  dplyr::summarise(n = length(unique(model))) %>%
  pull(n) %>%
  max() 
...
```{r prediction-plots, echo = FALSE, results='asis', out.width="100%", fig.height = ht}
